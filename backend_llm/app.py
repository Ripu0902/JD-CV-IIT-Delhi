...
def init_client():
    try:
        api_key = "4e960d3a-7383-467b-b729-b005ba6c3638"
        if not api_key:
...
def process_resume_batches(job_description, resume_files, batch_size=2):
    """
    ...
    # Ensure batch size is small to avoid context limitations
    batch_size = min(batch_size, 2)  # Maximum 2 resumes per batch to avoid context issues
